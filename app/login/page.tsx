...
import { DiseaseCard } from '@/components/DiseaseCard' // your existing card component
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchDiseases = async () => {
      ...
      if (!error && data) setDiseases(data as Disease[])
      setLoading(false)
    }
    fetchDiseases()
  }, [supabase])
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        {loading ? (
          <p className="py-20 text-center text-sm text-zinc-400">Loading knowledge base…</p>
        ) : filteredDiseases.length === 0 ? (
          <p className="py-20 text-center text-sm text-zinc-400">No diseases match your search.</p>
        ) : (
